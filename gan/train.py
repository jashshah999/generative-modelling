	...
	while iters < num_iterations:
		for train_batch in train_loader:
			with torch.cuda.amp.autocast(enabled=amp_enabled):
				...
				# interpolated_batch = interpolate_latent_space(gen, "./viz/interpolated.png")
				# discriminator_output_interpolated = disc(interpolated_batch)
				# x = gen(train_batch)
				# x = disc(x)
				gen_output = gen(train_batch.shape[0])
				disc_output_train_batch = disc(train_batch)
				disc_output_gen_data = disc(gen_output.detach())
			...
			if iters % 5 == 0:
				...
				# gen_out = gen(train_batch.shape[0])
					gen_output = gen(train_batch.shape[0])
					# disc_output_train_batch = disc(train_batch)
					# disc_output_gen_data = disc(gen_output)
					disc_output_gen_data = disc(gen_output)
					generator_loss = gen_loss_fn(disc_output_gen_data)
			...
			if iters % log_period == 0 and iters != 0:
				with torch.no_grad():
					with torch.cuda.amp.autocast(enabled=amp_enabled):
						...
						# out = generated_samples - min_val
						# max_val = out.max()
						# generated_samples = out/max_val
						generated_samples_min,_ = generated_samples.view(train_batch.shape[0],3,-1).min(dim=2)
						generated_samples = generated_samples - generated_samples_min.unsqueeze(-1).unsqueeze(-1)
						generated_samples_max,_ = generated_samples.view(train_batch.shape[0],3,-1).max(dim=2)
